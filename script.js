(() => {
  ...
  const HOBBY_EMOJI = {
    ...
    baking:"🧁", dance:"💃", fitness:"💪", makeup:"💄",
    nature:"🌿", boardgames:"🎲", streaming:"📱",
  };
  ...
  const progressContainer = document.querySelector('.bar[role="progressbar"]');
  ...
  function setProgress(pct) {
    ...
    if (progressContainer) {
      ...
      progressContainer.setAttribute('aria-valuetext', `${Math.round(p)} percent`);
    }
    if (p >= 100) {
  ...
  on($('#panel-signup'), 'submit', async (e) => {
    ...
    const pw       = toStr(els.suPassword?.value);
    setText(els.suStatus, 'Starting onboarding…');
    openOnboardingHobbies(); // jump straight to hobbies
  });
  ...
  function setStep(n) {
    ...
    if (els.stepAvatars)  els.stepAvatars.hidden  = n !== 4;
  }
  ...
  function initChipGroup(group, opts = {}) {
    ...
    function updateProgress() {
      ...
      if (nextBtn) {
        const isDisabled = values.size < min;
  ...
  // ----- Hobbies step: init + disable Next until min reached
  if (els.nextToGenres) {
    els.nextToGenres.disabled = true;
    els.nextToGenres.classList.add('is-disabled');
  }
  ...
  on(els.nextToGenres, 'click', (e) => {
    e.preventDefault();
    if (els.nextToGenres.disabled) return;

    // Inject Genres progress UI (if not already), then re-query fresh nodes
    ensureProgressUI(els.stepGenres, 'genreCount', 'genreProgress', MIN_GENRES);
    const freshGenreCount    = document.getElementById('genreCount');
    const freshGenreProgress = document.getElementById('genreProgress');
    ...
    if (!window.__genresInitDone) {
      ...
      window.__genresGroup = initChipGroup(els.genreChoices, {
        min: MIN_GENRES,
        max: Infinity,
        countEl: freshGenreCount,
        fillEl:  freshGenreProgress,
  ...
  // ----- Save preferences (Genres “Continue”)
  on(els.savePrefs, 'click', async (e) => {
    e.preventDefault(); // make sure we never submit a form / reload

    const hobbiesSelected = hobbies.values;
    const genresSelected  = (window.__genresGroup?.values ?? []);

    if (hobbiesSelected.length < MIN_HOBBIES || genresSelected.length < MIN_GENRES) {
      setText(els.prefsStatus, `Pick at least ${MIN_HOBBIES} hobbies and ${MIN_GENRES} genres.`);
      return;
    }

    // Persist locally first so suggestions always work
    try {
      localStorage.setItem('selectedHobbies', JSON.stringify(hobbiesSelected));
      localStorage.setItem('selectedGenres',  JSON.stringify(genresSelected));
    } catch {}
    ...
    let failed = false;
    try {
      if (typeof window.FirebaseAPI?.savePreferences === 'function') {
        await window.FirebaseAPI.savePreferences({ hobbies: hobbiesSelected, genres: genresSelected });
      } else {
        // dev fallback to simulate latency
        await new Promise(r => setTimeout(r, 300));
      }
    } catch (err) {
      failed = true;
      console.warn('savePreferences failed:', err);
    } finally {
      if (els.savePrefs) els.savePrefs.disabled = false;
    }

    setText(els.prefsStatus, failed ? 'Saved locally. Sync failed; will retry later.' : 'Preferences saved!');
    goToAvatars(); // advance regardless of backend result
  });
...
const BOOKS = [
  { id:'g1',  title:"Ready Player One", author:"Ernest Cline", pages:384, img:"assets/ready-player-one.jpg", tags:['gaming','scifi','vr','quests','nostalgia'] },
  { id:'g2',  title:"Warcross", author:"Marie Lu", pages:416, img:"assets/warcross.jpg", tags:['gaming','coding','esports','mystery'] },
  { id:'m1',  title:"Dumplin’", author:"Julie Murphy", pages:400, img:"assets/dumplin.jpg", tags:['music','coming-of-age','pageant','friendship'] },
  { id:'m2',  title:"This Song Will Save Your Life", author:"Leila Sales", pages:288, img:"assets/this-song-will-save-your-life.jpg", tags:['music','dj','identity','contemporary-ya'] },
  { id:'a1',  title:"Eliza and Her Monsters", author:"Francesca Zappia", pages:385, img:"assets/eliza-and-her-monsters.jpg", tags:['art','webcomics','mental-health','realistic','fanfiction'] },
  { id:'a2',  title:"The Art of Secrets", author:"James Klise", pages:256, img:"assets/the-art-of-secrets.jpg", tags:['art','mystery','thriller'] },
  { id:'s1',  title:"The Crossover", author:"Kwame Alexander", pages:240, img:"assets/crossover.jpg", tags:['sports','basketball','poetry'] },
  { id:'s2',  title:"Patina", author:"Jason Reynolds", pages:240, img:"assets/patina.jpeg", tags:['sports','track','friendship'] },
  { id:'f1',  title:"The Fashion Committee", author:"Susan Juby", pages:352, img:"assets/fashion-comittee.jpg", tags:['fashion','school','drama'] },
  { id:'f2',  title:"Project Runway Junior Companion Books", author:"Various", pages:224, img:"assets/project_runway.jpg", tags:['fashion','nonfiction','inspiration'] },
  { id:'c2',  title:"Little Brother", author:"Cory Doctorow", pages:384, img:"assets/little-brother.jpg", tags:['coding','hacking','activism','scifi'] },
  { id:'an1', title:"The One and Only Ivan", author:"Katherine Applegate", pages:336, img:"assets/the_one_and_only_ivan.jpg", tags:['animals','friendship','heartfelt'] },
  { id:'w1',  title:"Fangirl", author:"Rainbow Rowell", pages:480, img:"assets/fangirl.jpg", tags:['writing','fanfiction','romance','contemporary-ya'] },
  { id:'w2',  title:"Words on Bathroom Walls", author:"Julia Walton", pages:304, img:"assets/bathroom-walls.jpg", tags:['writing','journal','mental-health'] },
  { id:'t1',  title:"Love & Gelato", author:"Jenna Evans Welch", pages:400, img:"assets/love_and_gelato.jpg", tags:['travel','romance','mystery'] },
  { id:'t2',  title:"The Gentleman’s Guide to Vice and Virtue", author:"Mackenzi Lee", pages:528, img:"assets/the_gentleman.jpg", tags:['travel','historical','adventure','romance'] },
  { id:'mv1', title:"Paper Towns", author:"John Green", pages:336, img:"assets/paper_towns.jpg", tags:['movies','mystery','adventure'] },
  { id:'mv2', title:"One of Us Is Lying", author:"Karen M. McManus", pages:416, img:"assets/lying.jpg", tags:['movies','thriller','mystery'] },
  { id:'p1',  title:"Hold Still", author:"Nina LaCour", pages:240, img:"assets/hold_still.jpg", tags:['photography','grief','healing'] },
  { id:'p2',  title:"The Girl in the Picture", author:"Alexandra Monir", pages:272, img:"assets/girl_in_picture.jpg", tags:['photography','mystery','thriller'] },
  { id:'b1',  title:"Love Sugar Magic", author:"Anna Meriano", pages:320, img:"assets/love_sugar_magic.jpg", tags:['baking','magic','family'] },
  { id:'b2',  title:"A Pho Love Story", author:"Loan Le", pages:416, img:"assets/pho.jpg", tags:['baking','cooking','romance','family'] },
  { id:'d1',  title:"Tiny Pretty Things", author:"Sona Charaipotra & Dhonielle Clayton", pages:448, img:"assets/tiny_pretty_things.jpg", tags:['dance','ballet','drama'] },
  { id:'d2',  title:"I’ll Be the One", author:"Lyla Lee", pages:336, img:"assets/be_the_one.jpg", tags:['dance','k-pop','competition','romance'] },
  { id:'fit1',title:"Turtles All the Way Down", author:"John Green", pages:304, img:"assets/all_the_way_down.jpg", tags:['fitness','mental-health','friendship'] },
  { id:'fit2',title:"Girls Guide to Conquering Life", author:"Erica & Jonathan Catherman", pages:224, img:"assets/girls_guide.jpg", tags:['fitness','growth','nonfiction'] },
  { id:'mu1', title:"Beauty Queens", author:"Libba Bray", pages:400, img:"assets/beauty_queens.jpg", tags:['makeup','satire','pageant'] },
  { id:'mu2', title:"The Summer I Turned Pretty", author:"Jenny Han", pages:304, img:"assets/the_summer.jpg", tags:['makeup','romance','summer'] },
  { id:'n1',  title:"The Wild Robot", author:"Peter Brown", pages:288, img:"assets/the_robot.jpg", tags:['nature','survival','tech'] },
  { id:'n2',  title:"Where the Crawdads Sing", author:"Delia Owens", pages:384, img:"assets/crawdad_sing.jpg", tags:['nature','mystery','older-teen'] },
  { id:'bg1', title:"The Westing Game", author:"Ellen Raskin", pages:192, img:"assets/westwing_game.jpg", tags:['boardgames','mystery','puzzle'] },
  { id:'bg2', title:"Slay", author:"Brittney Morris", pages:352, img:"assets/slay.jpg", tags:['boardgames','gaming','strategy'] },
  { id:'sm1', title:"Follow Me", author:"Sara Shepard", pages:320, img:"assets/follow_me.jpg", tags:['streaming','social-media','thriller'] },
...
function populateRail() {
  ...
  displayBooks.forEach(({ book, matched }) => {
    frag.appendChild(renderTile(book, matched));
  });
